...
export function QRCodeView({ workspace, config }: QRCodeViewProps) {
  const [copiedField, setCopiedField] = useState<string | null>(null);

  // Use checkInPath from config, or default pattern
  const pathTemplate = config.checkInPath || '/checkin?store={workspaceId}';
  const urlPath = pathTemplate.replace('{workspaceId}', workspace.id);
  const checkinUrl = typeof window !== 'undefined'
    ? `${window.location.origin}${urlPath}`
    : `https://diemdanh.net${urlPath}`;

  const workspaceLabel = config.workspaceLabel || 'Workspace';
      ...
      if (ctx) {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            link.href = url;
            link.download = `qr-${workspace.name.replace(/\s+/g, '-').toLowerCase()}.png`;
            link.click();
  ...
  function printQRCode() {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>QR Code - ${workspace.name}</title>
          <style>
            body {
        ...
        <body>
          <div class="container">
            ...
            <p class="subtitle">${workspaceLabel}</p>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-4 sm:mb-6">
          ...
          <p className="text-sm sm:text-base text-gray-600">
            In hoặc chia sẻ mã QR này để {peopleLabel.toLowerCase()} có thể điểm danh dễ dàng
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-8 border-2 border-gray-200">
          <div className="text-center">
            {/* Workspace Name */}
            <div className="mb-4 sm:mb-6">
              <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-1">{workspace.name}</h3>
              <p className="text-xs sm:text-sm text-gray-600">{workspaceLabel}</p>
            </div>
            ...
            <div className="mt-4 sm:mt-6 bg-green-50 rounded-lg p-3 sm:p-4">
              ...
              <ol className="text-xs sm:text-sm text-green-700 text-left space-y-1 max-w-md mx-auto">
                <li>1. Quét mã QR bằng camera điện thoại</li>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-xs sm:text-sm text-blue-800">
              ...
              <ul className="space-y-1 list-disc list-inside">
                <li>Cần đăng nhập để điểm danh</li>
                <li>Mã QR này chỉ hoạt động cho {workspaceLabel.toLowerCase()} này</li>
                <li>Bạn có thể in và dán mã QR tại vị trí</li>
              </ul>
            </div>
          </div>
        </div>
